...
This file contains two configuration approaches:
1. A Config class that securely loads sensitive data from environment variables
2. Constants for application-wide settings organized by category

For sensitive information like API keys and passwords, use Config().get_api_key()
For general settings, use the constants directly.
"""
...
class Config:
    ...
    def _load_config(self):
        ...
        for key in ["WEATHER_API_KEY", "DATABASE_PASSWORD", "ARDUINO_SECRET"]:
            value = os.getenv(key)
            if value:
...
DATABASE_PASSWORD = Config().get_api_key("DATABASE_PASSWORD")
